...
QString BaseDevice::ConnectDevice( typeConnect tC , QString typeAdder){
    ...
    if (status < VI_SUCCESS)
        return(outBuffer = ErrorFunction(status));
    flag_connect = true;
    viSetAttribute(vi,VI_ATTR_TMO_VALUE,default_timeout);
    viSetAttribute(vi,VI_ATTR_TERMCHAR,default_terminate_char);
...
/*===========================================================================*/
bool BaseDevice::WriteCommand(QString command,QString& ret,int tmo){

    ret="";
    if( flag_connect == false)
    {
        ret=" Not ConnectDevice";
        return false;
    }
    ...
    viSetAttribute(vi,VI_ATTR_TMO_VALUE,tmo);
    status = viWrite(vi, (ViBuf)cmd, size, &retCnt);
    viSetAttribute(vi,VI_ATTR_TMO_VALUE,default_timeout);
    //status=viQueryf(vi,cmd,buffer);
    delete[] cmd;
    if (status < VI_SUCCESS)
    {
        ret = ErrorFunction(status);
        return false;
    }
...
/*===========================================================================*/
bool BaseDevice::ReadDevice( uint count,QString& ret,int tmo){
...
//    }
    ...
    while(retCnt>=count)
    {
        ...
        ret.append(QString(QByteArray(buffer)));
        if((VI_ERROR_TMO==status&&retCnt)||VI_SUCCESS==status)
        {
            delete[] buffer;
            viSetAttribute(vi,VI_ATTR_TMO_VALUE,tmo);
            return true;
...
/*===========================================================================*/
bool BaseDevice::IDN(QString& ret){
    if(!WriteCommand("*idn?\n",ret))
    {
        return false;
    }
    if(!ReadDevice(65,ret))
    {
       return false;
    }
    return true;
...
/*===========================================================================*/
bool BaseDevice::RST(QString& ret){
    if(!WriteCommand("*rst\n",ret))
    {
        return false;
    }
...
/*===========================================================================*/
bool BaseDevice::TST(QString& ret){
    if(!WriteCommand("*tst\n",ret))
    {
        return false;
    }
...
/*===========================================================================*/
bool BaseDevice::WAI(QString& ret){
    if(!WriteCommand("*wai\n",ret))
    {
        return false;
    }
...
/*===========================================================================*/
bool BaseDevice::OPC(QString& ret){
    if(!WriteCommand("*OPC?\n",ret))
    {
        return false;
    }
    ...
    if(ret.contains("0"))
    {
        ret="Operation is not ready";
        return false;
    }
    return true;
}
/*===========================================================================*/
bool BaseDevice::BUSY(QString& ret){
    if(!WriteCommand("*BUSY?\n",ret))
    {
        return false;
    }
    if(!ReadDevice(2,ret))
...
/*===========================================================================*/
QByteArray BaseDevice::ReadDevice_Array( uint count,int tmo){
   // char* buffer = new char[count]{};
    ViByte *buffer = new ViByte[count];
    ...
    if( flag_connect == false){
        delete[] buffer;
        buffer_array[0] = -1;
        return buffer_array;//(" Not ConnectDevice");
    }
    viSetAttribute(vi,VI_ATTR_TMO_VALUE,tmo);
